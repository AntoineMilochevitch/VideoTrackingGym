...
def run_mp(input_stream1, input_stream2, P0, P1, nbRep, nbSet, exoType):
    #input video stream
    ...
    while set_count > 0:
        ...
        if exoType == 1:
            left_position, right_position = curlDetection.main(frame_p3ds,counter)
            if start_left_position == "Down" and left_position == "Up":
                rep_count += 1
                start_left_position = "Up"
            elif start_left_position == "Up" and left_position == "Down":
                start_left_position = "Down"
            if start_right_position == "Down" and right_position == "Up":
                rep_count += 1
                start_right_position = "Up"
            elif start_right_position == "Up" and right_position == "Down":
                start_right_position = "Down"
        ...
        # Time required for each frame to achieve 15 fps
        required_time = 1.0 / 15

        # If frame processing time is less than required time, wait for the remaining time
        if frame_time < required_time:
            time.sleep(required_time - frame_time)
...
def main(cam1, cam2, exoType, nbSet, nbRep) :
    input_stream1 = cam1
    input_stream2 = cam2
    ...
    run_mp(input_stream1, input_stream2, P0, P1, nbRep, nbSet, exoType)
